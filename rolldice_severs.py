...
def roll_dice(diceCount, diceSides):
    return [random.randint(1, diceSides) for _ in range(diceCount)]


diceCount = int(input("How many Dice would you like to roll?"))
diceSides = int(input("How many sides on each dice?"))


results = roll_dice(diceCount, diceSides)
if diceSides <= 1 or diceCount == 0:
    print ("Sides must be greater than 1 and dice count greater than 0") 
else: print (f"Here are the results {results}")
